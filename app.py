...
def polly():
    ...
    res = po.synthesize_speech(
        Text=upname, OutputFormat="mp3", VoiceId='Aditi')
...
@app.route('/youtubedown', methods=['POST'])
def youtubedown():
    link = request.form.get("link")
    yt = YouTube(link)
    video = yt.streams.get_highest_resolution()
    video.download()
    return render_template("output.html", response="Video SAVED")


@app.route('/text', methods=['POST'])
def text():
    msg = request.form.get("tex")
    pywhatkit.text_to_handwriting(msg, "hand.png")
    return render_template("output.html", response="Converted")
...
def watsapp():
    ...
    minu = request.form.get("min")
    pywhatkit.sendwhatmsg(mob, msg, int(hr), int(minu))
    return render_template("output.html", response="MSG SENT")
